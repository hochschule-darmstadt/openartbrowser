...
let movementsReadStream = fs.createReadStream(movementFilePath, {
  flags: 'r',
  encoding: 'utf8'
});
    ...
    .on('end', function() {
      ...
      _.each(movementMonsterObj, function(obj) {
        _.each(obj, function(val, key) {
          if (key === 'id') {
            if (!_.isUndefined(movementsCounter.get(val))) console.log('this key already exits: ' + val);
        ...
        _.each(obj, function(val, key) {
          if (key === 'movements') {
            _.each(val, function(movement) {
              ...
              } else movementsCounter.set(movement, movementsCounter.get(movement) + 1);
            });
          }
        });
      });
