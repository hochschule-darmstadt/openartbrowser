...
let materialReadStream = fs.createReadStream(materialFilePath, {
  flags: 'r',
  encoding: 'utf8'
});
    ...
    .on('end', function() {
      ...
      _.each(materialMonsterObj, function(obj) {
        _.each(obj, function(val, key) {
          if (key === 'id') {
            if (!_.isUndefined(materialsCounter.get(val))) console.log('this key already exists: ' + val);
        ...
        _.each(obj, function(val, key) {
          if (key === 'materials') {
            _.each(val, function(material) {
              ...
              } else materialsCounter.set(material, materialsCounter.get(material) + 1);
            });
          }
        });
      });
