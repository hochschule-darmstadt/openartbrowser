...
const destLocationFilePath = 'locations_rank.json';

const fs = require('fs');
const _ = require('lodash');

let locationReadStream = fs.createReadStream(locationFilePath, {
  flags: 'r',
  encoding: 'utf8'
});
    ...
    .on('end', function() {
      ...
      _.each(locationMonsterObj, function(obj) {
        _.each(obj, function(val, key) {
          if (key === 'id') {
            if (!_.isUndefined(locationsCounter.get(val))) console.log('this key already exits: ' + val);
        ...
        _.each(obj, function(val, key) {
          if (key === 'locations') {
            _.each(val, function(location) {
              ...
              } else locationsCounter.set(location, locationsCounter.get(location) + 1);
            });
          }
        });
      });
...
function writeArtists() {
  ...
  fs.writeFile(destLocationFilePath, JSON.stringify(locationMonsterObj), 'utf-8', function(err) {
    if (err) throw err;
