...
let genreReadStream = fs.createReadStream(genresFilePath, {
  flags: 'r',
  encoding: 'utf8'
});
    ...
    .on('end', function() {
      ...
      _.each(genreMonsterObj, function(obj) {
        _.each(obj, function(val, key) {
          if (key === 'id') {
            if (!_.isUndefined(genresCounter.get(val))) console.log('this key already exits: ' + val);
          ...
          if (key === 'genres') {
            _.each(val, function(genre) {
              if (_.isUndefined(genresCounter.get(genre))) {
                undefinedData++;
                console.log(genre);
              } else genresCounter.set(genre, genresCounter.get(genre) + 1);
            });
          }
        });
      });
