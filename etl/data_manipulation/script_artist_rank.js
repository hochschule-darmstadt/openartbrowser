...
let artistReadStream = fs.createReadStream(artistFilePath, {
  flags: 'r',
  encoding: 'utf8'
});
    ...
    .on('end', function() {
      ...
      _.each(artistMonsterObj, function(obj) {
        _.each(obj, function(val, key) {
          if (key === 'id') {
            if (!_.isUndefined(artistsCounter.get(val))) console.log('this key already exits: ' + val);
          ...
          if (key === 'artists') {
            _.each(val, function(artist) {
              if (_.isUndefined(artistsCounter.get(artist))) {
                undefinedData++;
              } else artistsCounter.set(artist, artistsCounter.get(artist) + 1);
            });
          }
        });
      });
