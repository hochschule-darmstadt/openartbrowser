...
interface EntityTab {
  type: EntityType;
  itemsFound: boolean; // boolean if searchResults exist for this EntityType
  icon: EntityIcon;
  active: boolean;
}
...
export class SearchResultComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.pipe(takeUntil(this.ngUnsubscribe)).subscribe(async params => {
      ...
      if (params.artist) {
        this.searchObjects.push(await this.getSearchResults(params.artist, EntityType.ARTIST, EntityIcon.ARTIST));
      }
      if (params.movement) {
        this.searchObjects.push(await this.getSearchResults(params.movement, EntityType.MOVEMENT, EntityIcon.MOVEMENT));
      }
      if (params.genre) {
        this.searchObjects.push(await this.getSearchResults(params.genre, EntityType.GENRE, EntityIcon.GENRE));
      }
      if (params.motif) {
        this.searchObjects.push(await this.getSearchResults(params.motif, EntityType.MOTIF, EntityIcon.MOTIF));
      }
      if (params.location) {
        this.searchObjects.push(await this.getSearchResults(params.location, EntityType.LOCATION, EntityIcon.LOCATION));
      }
      if (params.material) {
        this.searchObjects.push(await this.getSearchResults(params.material, EntityType.MATERIAL, EntityIcon.MATERIAL));
      }
      this.searchResults = await this.getSliderItems(this.searchObjects, this.searchTerms);
   ...
   */
  private async getSearchResults<T>(ids: [], type: EntityType, icon: EntityIcon): Promise<SearchObject> {
   ...
   */
  private async getSliderItems(results: SearchObject[], terms: string[]): Promise<Artwork[]> {
   ...
   */
  private addTab(type: EntityType, itemsFound: boolean = false, active: boolean = false) {
